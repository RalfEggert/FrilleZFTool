...
class ModuleGenerator
{
     ...
     * @var boolean
     */
    protected $flagCreateApiDocs = true;
    /**
     * @var RequestOptions
     */
    ...
    function __construct(RequestOptions $requestOptions)
    {
        ...
        $this->flagCreateApiDocs = !$this->requestOptions->getFlagNoApiDocs();
    }
    ...
    public function createControllerFactory()
    {
        ...
        $code->addMethodFromGenerator(
            $this->generateCreateServiceMethod(
                $controllerClass, 'controllerLoader', 'controller'
            )
        );
    ...
    public function createViewHelper()
    {
        ...
        $code->addMethodFromGenerator(
            $this->generateViewHelperMethod($viewHelperClass)
        );
        ...
        if ($this->flagCreateApiDocs) {
            $code->setDocBlock(
                new DocBlockGenerator(
                    'View helper ' . $viewHelperClass,
                    'Please add a proper description for the '
                    . $viewHelperClass . ' view helper',
                    array(
                        $this->generatePackageTag($moduleName),
                    )
                )
    ...
    public function createViewHelperFactory()
    {
        ...
        if (file_exists($factoryFilePath)) {
            throw new GeneratorException(
                'The factory for this view helper exists already.'
            );
        }
        ...
        $code->addMethodFromGenerator(
            $this->generateCreateServiceMethod(
                $viewHelperClass, 'viewHelperManager', 'viewHelper'
            )
        );
        ...
        if ($this->flagCreateApiDocs) {
            $code->setDocBlock(
                new DocBlockGenerator(
                    'Factory for ' . $viewHelperClass,
                    'Please add a proper description for the '
                    . $viewHelperClass . ' factory',
                    array(
                        $this->generatePackageTag($moduleName),
                    )
                )
    ...
    /**
     * Generate the view helper invoke method
     *
     * @return MethodGenerator
     */
    protected function generateViewHelperMethod($viewHelperClass)
    {
        // create method
        $method = new MethodGenerator();
        $method->setName('__invoke');
        $method->setBody(
            'return __CLASS__ . \' was called<br>\';'
        );
        ...
        if ($this->flagCreateApiDocs) {
            $method->setDocBlock(
                new DocBlockGenerator(
                    'Call view helper ' . $viewHelperClass,
                    'Please add a proper description for this method',
                    array(
                        $this->generateReturnTag('string'),
                    )
                )
            );
        }
     ...
     * @return MethodGenerator
     */
    protected function generateCreateServiceMethod(
        $objectClass, $serviceManagerClass, $instanceName
    )
    {
        // init body
        $methodBody = array();
